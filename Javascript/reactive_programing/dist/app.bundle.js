...
/***/ function(module, exports, __webpack_require__) {
	...
	var numbers = [33, 11, 22, 44, 55, 66, 77, 88];
	var numbers$ = _Rx2.default.Observable.from(numbers);
	numbers$.subscribe(console.log, function (err) {
	  return console.log(err);
	}, function () {
	  return console.log('completed');
	});
	...
	var posts = [{ name: 'john', msg: 'hello' }, { name: 'jane', msg: 'hii' }, { name: 'kate', msg: 'hola' }];
	var posts$ = _Rx2.default.Observable.from(posts);
	posts$.subscribe(console.log, console.log, function () {
	  return console.log('done');
	});
	posts$.subscribe(function (post) {
	  (0, _jquery2.default)("#ul").append('<li><h3>' + post.name + ' says ' + post.msg + '</h3></li>');
	}, console.log, function () {
	  (0, _jquery2.default)("#ul").append('end of posts');
	});

	// from a javascript collection -> set and map
	var set = new Set(['hello', 44, { key: 'value' }]);
	var set$ = _Rx2.default.Observable.from(set);
	set$.subscribe(console.log, console.log, function () {
	  return console.log('SET COMPLETE');
	});

	var map = new Map([[12, 33], [{ a: 1 }, 5], ['txt', 55], ['a', [1, 2, 3]]]);
	var map$ = _Rx2.default.Observable.from(map);
	map$.subscribe(console.log, console.log, function () {
	  return console.log('MAP COMPLETE');
	});
	...
	source$.catch(function (err) {
	  ...
	  return console.log('Received from observable: ' + number);
	}, console.log, function () {
	...
	promise.then(console.log, //resolve function
	console.log // reject function
	);

	var promise$ = _Rx2.default.Observable.fromPromise(promise);

	promise$.subscribe(function (msg) {
	  return console.log('message from observable -> ' + msg);
	}, console.log, function () {
	  return console.log('complete');
	});
	...
	var github$ = _Rx2.default.Observable.fromEvent((0, _jquery2.default)('#github-input'), 'keyup');
	github$.subscribe(function (event) {
	  getUser(event.target.value).then(function (response) {
	    console.log(response);
	    var _response$data = response.data,
	        name = _response$data.name,
	        followers = _response$data.followers;

	    if (name == null) {
	      (0, _jquery2.default)("#github-data").text('Username not found');
	    } else {
	      (0, _jquery2.default)('#github-data').text('My name is ' + name + ' and I have ' + followers + ' followers');
	    }
	  });
	});
